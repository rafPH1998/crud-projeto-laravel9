...
<form action="{{route('comments.index', $user->id)}}" method="get" class="py-5">
    <input type="text" name="search" placeholder="Pesquisar comentário" class="md:w-1/6 bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500">
...
<table class="min-w-full leading-normal shadow-md rounded-lg overflow-hidden mt-2">
  <thead>
      <tr>
      ...
      @foreach ($comments as $comment)
          <tr>
              ...
              <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                  <a href="{{route('comments.edit', ['user' => $user->id, 'id' => $comment->id])}}" class="bg-green-200 rounded-full py-2 px-6 hover:bg-green-300">Editar</a>
              </td>
          </tr>
      @endforeach
  </tbody>
